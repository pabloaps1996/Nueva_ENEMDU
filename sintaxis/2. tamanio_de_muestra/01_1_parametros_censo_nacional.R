...
parametros_nac <- aggr_nac %>%
  mutate(tasa_de = N_per_de/N_per_pea,
         tasa_do = N_per_do/N_per_ft,
         r_pea = N_per_pea/N_per,
         r_ft = N_per_ft/N_per) %>%
  full_join(nac_rho_de,
            by = "nac") %>%
  full_join(nac_rho_do,
            by = "nac") %>%
  select(nac, N_per, b = promedio_per_viv,
         N_per_pea, N_per_de, tasa_de, r_pea, rho_de,
         N_per_ft, N_per_do, tasa_do, r_ft, rho_do)

saveRDS(parametros_nac, "intermedios/2. tamanio_de_muestra/parametros_nac.rds")
