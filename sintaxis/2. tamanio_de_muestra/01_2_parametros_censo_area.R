...
aggr_area <- poblacion2022 %>%
  as.data.frame() %>%
  group_by(area, id_viv) %>%
  summarise(N_per = n(),
...
parametros_area <- aggr_area %>%
  mutate(tasa_de = N_per_de/N_per_pea,
         tasa_do = N_per_do/N_per_ft,
         r_pea = N_per_pea/N_per,
         r_ft = N_per_ft/N_per) %>%
  full_join(area_rho_de,
            by = "area") %>%
  full_join(area_rho_do,
            by = "area") %>%
  select(area, N_per, b = promedio_per_viv,
         N_per_pea, N_per_de, tasa_de, r_pea, rho_de,
         N_per_ft, N_per_do, tasa_do, r_ft, rho_do)

saveRDS(parametros_area, "intermedios/2. tamanio_de_muestra/parametros_area.rds")
